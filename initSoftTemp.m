...
particleMaster.interactLrV = { {'ss2d', 1, 2, 1, 1.855},...
{'pa2d', [0.1 ] } }; 
% External potential (cell of cells): 
%  { {'linV', dim, a}, {'quadV', dim, a}, {'nemV', es1} }
particleMaster.externalV = {}; % External potential 
%%%%%%%%% System %%%%%%%%%%%%%%%%
systemMaster.n1 = [256]; % Gridpoints in x dir (vec)
systemMaster.n2 = [256]; % Gridpoints in y dir (vec) 
systemMaster.n3 = [128]; % Gridpoints in angle (vec)
systemMaster.bc = pi / 4 * [3]; % Scaled concentration (vec)
systemMaster.l1 = [10];  % Box length (vec)
systemMaster.l2 = [10];  % Box length (vec)
systemMaster.l3 = 2 * pi;
systemMaster.tmp = 1;   % Temperature
%%%%%%%%%%%%%%% Time %%%%%%%%%%%%%%%%%%%%%%%%%%
timeMaster.dt         = 1e-4; % time step
