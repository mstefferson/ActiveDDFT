...
%particleMaster.vD   = [0 10]; % Driving velocity (vec)
...
%particleMaster.interactLrV = { {'quadV', 2, 0.0001}, {'nem2D', 0.001, pi/2} }; 
% External potential (cell of cells): 
%  { {'linV', dim, a}, {'quadV', dim, a}, {'nemV', es1} }
%particleMaster.externalV = { {'quadV', 1, 0.01}, {'nemV', .001, pi/2 } }; 
particleMaster.externalV = { }; 
%%%%%%%%% System %%%%%%%%%%%%%%%%
systemMaster.n1 = [100]; % Gridpoints in x dir (vec)
systemMaster.n2 = [100]; % Gridpoints in y dir (vec) 
systemMaster.n3 = [100]; % Gridpoints in angle (vec)
systemMaster.bc = [1.65]; % Scaled concentration (vec)
...
timeMaster.t_rec      = 0.25;  % time elapsed before recording
timeMaster.t_write    = 1;  % time elapsed before writing to file
timeMaster.t_tot      = 6.0;  % total run time
...
% 7: Crystal
rhoInitMaster.IntCond   = [6]; % IC indicator (vec)
rhoInitMaster.LoadName  = 'rho160Vd30Band.mat'; % File name to load if IC 3
