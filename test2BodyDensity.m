...
for ii = 1:systemObj.n1
  ...
  inds1 = mod( (ii-1) + indsSpread1 - 1, systemObj.n1 ) + 1;
  for jj = 1:systemObj.n2
    % inds for loop over delta y
    inds2 = mod( (jj-1) + indsSpread2 - 1, systemObj.n2 ) + 1;
    for kk = 1:totalInds1
      for ll = 1:totalInds2
        % plug into
        vec2Int = rhoTemp(ii, jj, :) .* rhoTemp( inds1(kk), ind2(ll), :) .* mayer( indsDelta1(kk), indsDelta2(ll), : );
        vec2Int = reshape( vec2Int, [1, systemObj.n3] );
        fullCorrelation2( ii, jj, inds1(kk), ind2(ll) ) = 2 * pi * trapz_periodic( phi, vec2Int );
        fullCorrelation2( ii, jj, inds1(kk), ind2(ll) ) = fullCorrelation2( ii, jj, inds1(kk), ind2(ll) ) ./ ( cTemp(ii,jj) .* cTemp(inds1(kk), inds2) );
      end
    end
  end
end
...
for ii = 1:totalInds1
  ...
  for jj = 1:totalInds2
    ...
    for kk = 1:systemObj.n1
      deltaK = mod( kk + (ii-1), systemObj.n1 );
      for ll = 1:systemObj.n2
        deltaL = mod( ll + (jj-1), systemObj.n2 );
...
aveCorrelation2 = aveCorrelation2 ./ 
% Average version 2
allInds1 = 1:systemObj.n1;
allInds2 = 1:systemObj.n2;
