...
runMaster.trialID = 2; % Trial Indicator
runMaster.runID   = 2; % Starting Run indicator *_trial.runID
%%%%%%%%%% Particles %%%%%%%%%%
...
particleMaster.vD   = [30]; % Driving velocity (vec)
...
systemMaster.bc = [1.35 1.65]; % Scaled concentration (vec)
systemMaster.l1 = [10];  % Box length (vec)
systemMaster.l2 = [10];  % Box length (vec)
systemMaster.l3 = 2 * pi;
systemMaster.tmp = 1;   % Temperature
%%%%%%%%%%%%%%% Time %%%%%%%%%%%%%%%%%%%%%%%%%%
timeMaster.dt         = 4e-3; % time step
timeMaster.t_rec      = 0.25;  % time elapsed before recording
timeMaster.t_write    = 1.0;  % time elapsed before writing to file
timeMaster.t_tot      = 10.0;  % total run time
