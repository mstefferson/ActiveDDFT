...
runMaster.trialID = 13; % Trial Indicator
...
particleMaster.vD   = [10]; % Driving velocity (vec)
...
%%%%%%%%% System %%%%%%%%%%%%%%%%
systemMaster.n1 = [160]; % Gridpoints in x dir (vec)
systemMaster.n2 = [160]; % Gridpoints in y dir (vec) 
systemMaster.n3 = [160]; % Gridpoints in angle (vec)
systemMaster.bc = [1.35]; % Scaled concentration (vec)
...
timeMaster.t_tot      = 4.0;  % total run time
...
%rhoInitMaster.intCond = {'lorenz', 1, 1, -2.5, 1, -2.5, 1, pi/4};
%rhoInitMaster.intCond = {'lorenz', 1, 1, 0, 1, 0, 1, 0};
...
%rhoInitMaster.perturb = { {'lorenz', 1, 1, 2.5, 1, -2.5, 1, 3*pi/4} };
% Save the Params
