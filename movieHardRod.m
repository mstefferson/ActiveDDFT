    ...
    for ii = 1:numDirs
      ...
      if movieflags.makeMovie
        if systemObj.n3 > 1
          ...
          % Save Name
          movStr = sprintf('OPmov_bc%.2f.fD%.1f_%.2d_%.2d.avi',...
          ...
          % Save Name
          movStr = sprintf('Cmov_bc%.2f.fD%.1f_%.2d_%.2d.avi',...
            systemObj.bc,particleObj.fD,runObj.trialID, runObj.runID);
          % make movie
      ...
      if movieflags.plotAmp
        ...
        savefig(gcf,figtl)
        figtl2 = sprintf('AmpFT_bc%.2f.fD%.0f_%.2d_%.2d',...
          systemObj.bc, particleObj.fD,runObj.trialID, runObj.runID);
      ...
      if movieflags.plotSlice
        sliceSaveTag = sprintf('SOP_bc%.2f.fD%.0f_%.2d_%.2d',...
          systemObj.bc, particleObj.fD,runObj.trialID, runObj.runID);
