...
for ii = 1:n1
  ...
  for jj = 1:n2
    ...
    num = trapz_periodic( phi, vec2Int, 3 );
    pDistNoNorm( ii, jj ) = trapz_periodic( x2, trapz_periodic( x1, num, 1), 2 );
  end
end
...
pDistNoNorm = circshift( circshift( pDistNoNorm, center1-1, 1 ), center2-1, 2 );
keyboard
% plot
if plotflag
  figure()
  ...
  title('pair distribution g(x,y): density dependent. normalized')
  xlabel('x'); ylabel('y')
  colorbar
  subplot(1,2,2)
  imagesc( x1, x2, pDistNoNorm )
  title('pair distribution g(x,y): density dependent. no norm')
  xlabel('x'); ylabel('y')
  colorbar
end
