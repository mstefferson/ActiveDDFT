...
if numRuns > 1
  ...
  parfor ii = 1:numRuns
    ...
    if strcmp( interLr, '_softshoulder' )
      lrStrTemp = [ interLr '_1il' num2str( paramLrL1(ii), '%.3g' ) '_2il' ...
        num2str( paramLrL2(ii), '%.3g' )...
        '_1ie' num2str( paramLrE1(ii), '%.3g' )  '_2ie' ...
        num2str( paramLrE2(ii), '%.3g' ) ];
    else
      lrStrTemp = lrStr;
    end
    % Name the file
    filename = [ 'Hr' ptype, interHb, lrStrTemp,  externalPot, ...
      ...
      '_bc' num2str( parambc(ii), '%.3g' ) '_vD' num2str( paramvD(ii), '%.3g' ) ...
